...
一组 Agent + 可选 Member，加一名**队长 Agent**。assignee picker 里可以直接选小队——任务会落到队长头上，由队长 `@` 派给合适的成员。详见 [11-squads.md](11-squads.md)。
...
- Project 上能挂 **ProjectResource**——`github_repo` 或 `local_directory`，自动注入到 Agent 的工作目录里。详见 [09-projects-and-resources.md](09-projects-and-resources.md)。
...
一条 issue 可以反复触发**多个**Task。同一个 Agent 在同一条 issue 上**最多 1 个活跃 Task**（数据库 unique index 强制）；不同 Agent 可以并发。详见 [08-tasks-and-runs.md](08-tasks-and-runs.md)。
...
下一步：[03-core-loop.md](03-core-loop.md) — Agent 接到任务后该走的标准 loop。
