...
> Agent 的对象模型在 [02-entities.md](02-entities.md)；运行时机制在 [01-architecture.md](01-architecture.md)；11 款 AI 编程工具的差异在 [12-providers.md](12-providers.md)。
...
> 经验法则：**身份/指令/工具/可见性有任何一项不同 → 新建。否则复用。** 用大量"近亲" Agent 互相 `@` 来拼工作流是反模式——优先用 Squad（[10-squads.md](10-squads.md)）或子 issue（[04-issue-lifecycle.md](04-issue-lifecycle.md)）拼。
...
| 决策 | 选项 | 怎么挑 |
|---|---|---|
| **Provider / 工具** | Claude Code / Codex / Copilot / Cursor / Gemini / Hermes / Kimi / Kiro CLI / OpenCode / OpenClaw / Pi | 看 [12-providers.md](12-providers.md)。**默认 Claude Code**：会话恢复真用、是 11 款里**唯一真消费 MCP** 的。需要 Gemini/Codex 才考虑别的。 |
...
| **Skills** | 一组 skill ID（**全量替换**，详见 [09-skills.md](09-skills.md)） | 至少把 multica-wiki 这种"通识 skill"挂上；任务专长 skill 按需追加 |
...
3. **加进 Squad**——做小队成员，由队长 Agent 路由分派。详见 [10-squads.md](10-squads.md)。
4. **Autopilot 触发**——cron / webhook 周期性触发它。**不会自动重试**。详见 [11-autopilots.md](11-autopilots.md)。

## 不在这一章里的细节

| 想查什么 | 去哪一章 |
|---|---|
| Skill 怎么写 / 导入 / 挂载顺序 / 风险 | [09-skills.md](09-skills.md) |
| 11 款工具的细节差异 / 会话恢复 / MCP | [12-providers.md](12-providers.md) |
| Task 状态机 / 失败重试 / resume | [07-tasks-and-runs.md](07-tasks-and-runs.md) |
