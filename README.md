...
references/            # 渐进披露式参考文档：Agent 用到再读，不会一次性塞进上下文
  ...
  15-build-a-skill.md    # 何时新建 vs 复用 Skill / 决策清单 / 步骤 + 验证 / 常见翻车（配 10-skills）
  16-build-a-squad.md    # 何时新建小队 / 队长 + 成员 / Squad Instructions / 验证 / 常见翻车（配 11-squads）
  17-build-an-autopilot.md  # 何时建 Autopilot / cron · webhook / 模式选择 / 验证 / 常见翻车（配 12-autopilots）
