...
| 2026-05-28 | `v0.3.10` 重构 M2 | 通用智能体(Opus4.7) | 把"动作章 ↔ 名词章"配对铺开：新增 `15-build-a-skill.md` / `16-build-a-squad.md` / `17-build-an-autopilot.md`，分别与 `10-skills.md` / `11-squads.md` / `12-autopilots.md` 配对。每章顶部显式指回名词章（"只想知道 X 是什么 / 有哪些类型，看 N-noun.md"）。**不删 / 不吸收**名词章——保留 wiki 属性；保持扁平 14+3+附录索引（不切 Part），与 M1 决定一致。SKILL.md / README.md / SOURCES.md / compatibility.md 同步引用。COSI-33 跟踪。 | n/a（结构 + 新章节，未跑评测） |
